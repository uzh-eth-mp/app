...
from app.web3.transaction_events.types import MintPairEvent, BurnPairEvent, SwapPairEvent, ContractEvent


@_event_mapper(ContractCategory.UNI_SWAP_V2_PAIR)
def _mint(contract: Contract, receipt: TxReceipt, block_hash: HexBytes) -> Generator[ContractEvent, None, None]:
    for eventLog in contract.events.Mint().processReceipt(receipt):
        ...
        yield MintPairEvent(
            contract_address=receipt["contractAddress"], sender=sender, amount0=amount0, amount1=amount1)


@_event_mapper(ContractCategory.UNI_SWAP_V2_PAIR)
def _burn(contract: Contract, receipt: TxReceipt, block_hash: HexBytes) -> Generator[ContractEvent, None, None]:
    ...
    for eventLog in contract.events.Burn().processReceipt(receipt):
        ...
        to = eventLog['args']['to']
        yield BurnPairEvent(
            contract_address=receipt["contractAddress"], src=sender, dst=to, amount0=amount0, amount1=amount1)
