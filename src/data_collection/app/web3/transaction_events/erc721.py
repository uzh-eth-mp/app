    ...
    for eventLog in contract.events.Transfer().processReceipt(receipt):
        if eventLog['event'] == 'Transfer':
            ...
            if dst in burn_addresses:
                yield BurnNonFungibleEvent(contract_address=receipt["contractAddress"], account=src, tokenId=token_id)
            elif src in burn_addresses:
                yield MintNonFungibleEvent(contract_address=receipt["contractAddress"], account=dst, tokenId=token_id)
            else:
                yield TransferNonFungibleEvent(
                    contract_address=receipt["contractAddress"], src=src, dst=dst, tokenId=token_id)
