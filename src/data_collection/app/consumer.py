...
class DataConsumer(DataCollector):
    ...
    async def start_consuming_data(self):
        ...
        """
        try:
            # Consume transaction hashes from Kafka indefinitely
            async for event in self.kafka_manager.start_consuming():
                # Get transaction hash from Kafka event
                tx_hash = event.value.decode()
                # Get transaction data
                tx_data: TransactionData = await self.node_connector.get_transaction_data(tx_hash)
                tx_receipt_data: TransactionReceiptData = \
                    await self.node_connector.get_transaction_receipt_data(tx_hash)
